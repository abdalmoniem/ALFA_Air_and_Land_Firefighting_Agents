...
class ActionExec(object):
    def __init__(self, client_topic='move_base'):
        print 'initializing action exec...'
        rospy.logdebug('initializing action exec...')

        self.client_topic = client_topic
        self.status_publisher = rospy.Publisher(
            'patrol_status', String, queue_size=10)
        self.directions_subscriber = rospy.Subscriber(
            'robot0/coordinator_directions', Pose2D, self.sendARobot)
        self.client = actionlib.SimpleActionClient(
            self.client_topic, MoveBaseAction)
        self.client.wait_for_server()

        print 'initialized action exec'
        rospy.logdebug('initialized action exec')

    def sendARobot(self, msg):
        pose = msg
        goal = MoveBaseGoal()
        # 1 map? or should there be 1 per core?
