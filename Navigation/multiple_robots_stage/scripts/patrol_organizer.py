...
#
    ...
    def __init__(self, availableRobots=['robot_0', 'robot_1'], listOfPositions={'robot_0': [(1, 0), (0, 0)], 'robot_1': [(1, 2), (1, 1)]}):
        ...
        for robotName in availableRobots:
            self.status_subscriber[robotName] = rospy.Subscriber(
                robotName + '/patrol_status', String, self.askForNextLocation)
            topic = robotName + '/coordinator_directions'
            self.robotPublishers[robotName] = rospy.Publisher(
                topic, Pose2D, queue_size=10, latch=True)
    ...
    def sendDirection(self, robot):
        ...
        self.nextPositionIndex[robot] = (
            self.nextPositionIndex[robot] + 1) % len(self.listOfPositions[robot])


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--robot_to_control", default="robot_0",
                        help="robot's name. should be unique in order to coordinate multiple robots. default: robot_0 ")
